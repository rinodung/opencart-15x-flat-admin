...
var UniteAdminRev = new function(){
	...
	********************************/
	...
	var openOldImageDialog = function(title,onInsert){
		var field = 'sds_rev_imgsrc';
		var img_src;
		$('#dialog').remove();
		$('body').prepend('<input type="hidden" name="sds_rev_imgsrc" value="" id="sds_rev_imgsrc"><div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?route=common/filemanager&token='+sds_rev_oc_token+'&field=' + encodeURIComponent(field) + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
		$('#dialog').dialog({
				...
				if ($('#' + field).attr('value')) {
					$.ajax({
						...
						success: function(data) {
							var sdssrc = $('#' + field).attr('value');
							img_src = sds_main_shop_up_url+sdssrc;
							onInsert(img_src,"");
						}
					});
				}
	...
	t.openAddImageDialog = function(title,onInsert,isMultiple){
		//image_upload('slide_thumb', 'slide_thumb_button_preview');
		// if(!title)
		// 	title = 'Select Image';
