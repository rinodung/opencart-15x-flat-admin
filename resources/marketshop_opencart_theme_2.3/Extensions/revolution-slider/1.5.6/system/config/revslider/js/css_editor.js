...
var UniteCssEditorRev = new function(){
	...
	var initEditorDialog = function(){
		...
		jQuery("#button_edit_css").click(function(){
			...
			if(!UniteLayersRev.getLayerGeneralParamsStatus()) return false; 

			if(jQuery.trim(jQuery('#layer_caption').val()).length == 0) return false;
				...
				open:function () {
					jQuery(this).closest(".ui-dialog")
						.find(".ui-button").each(function(i) {
						   ...
						   jQuery('.css_editor_novice_wrap').css('height','auto');					   						   
						   jQuery('.ui-accordion-content').css('height','auto');
						   jQuery('.ui-widget-overlay').css('z-index','2');
	 ...
	 */
	var setallcssdatasample = function(){
		jQuery('.css-edit-enable').hide();
			jQuery('#css_editor_allow').text('');
