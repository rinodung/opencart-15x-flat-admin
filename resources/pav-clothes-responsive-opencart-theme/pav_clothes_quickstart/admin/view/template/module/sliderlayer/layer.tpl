		...
		<div class="container-fluid">
			<div class="pull-right">
				<a class="btn btn-primary" onclick="$('#slider-form').submit();" class="button"><i class="fa fa-save"></i> <?php echo $button_save; ?></a>
      			...
      			<a class="btn btn-success" id="btn-preview-ingroup" href="<?php echo  $objurl->link('module/pavsliderlayer/previewGroup', 'id='.$group_id.'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL');?>" class="button green" id="preview"><i class="fa fa-play"></i>  <?php echo $objlang->get('Preview Sliders In Group');?></a> |

      			<a class="btn btn-default" id="btn-preview-ingroup" href="<?php echo  $objurl->link('module/pavsliderlayer', 'id='.$group_id.'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL');?>" class="button green" id="preview"><i class="fa fa-mail-reply"></i> <?php echo $objlang->get('Back to Edit Group');?></a> |
			...
			<div class="panel-body">
				<ul id="languagetabs" class="nav nav-tabs" role="tablist">
					...
					<li <?php echo $class; ?> data-value="<?php echo $language['language_id']; ?>">
						<?php $url = $objurl->link('module/pavsliderlayer/layer', '&group_id='.$group_id.'&lang='.$language['language_id'].'&token=' . $objsession->data['token'], 'SSL'); ?>
						<a href="<?php echo $url; ?>"><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?></a></li>
					<?php } ?>
				</ul>
				<div class="pull-right">
				...
				<br>
				<h3><?php echo $objlang->get('List Of Sliders In Group');?>: <a href="<?php echo  $objurl->link('module/pavsliderlayer', 'id='.$group_id.'&token=' . $objsession->data['token'], 'SSL');?>"><span><?php echo $sliderGroup['title'];?></span></a></h3>
	    	 	<div>
	    		...
	    		<div class="group-sliders clearfix">
	    			<div class="new-slider-item">
		    			<a href="<?php echo  $objurl->link('module/pavsliderlayer/layer', 'group_id='.$group_id.'&token=' . $objsession->data['token'], 'SSL')?>">
		    			</a>
	    			...
	    			<?php foreach( $sliders as $slider )  { ?>
	    				<div class="slider-item <?php echo ( $slider['id'] == $slider_id ? 'active':'');?>" id="slider_<?php echo $slider['id'];?>">
	    					<a class="image" href="<?php echo  $objurl->link('module/pavsliderlayer/layer', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>">
	    						<img class="img-responsive" src="<?php echo HTTP_CATALOG."image/".$slider["image"];?>" height="86"/>
	    					</a>
	    					<a  title="<?php echo $objlang->get('text_clone_this');?>" class="slider-clone" href="<?php echo  $objurl->link('module/pavsliderlayer/copythis', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>"><span>Clone</span></a>
	    					<a  title="<?php echo $objlang->get('text_delete_this');?>" class="slider-delete" href="<?php echo  $objurl->link('module/pavsliderlayer/deleteslider', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>" onclick="return confirm('<?php echo $objlang->get('text_confirm_delete');?>')"><span>Delete</span></a>

	    					<a class="slider-status<?php if( !$slider['status']) { ?> slider-status-off <?php } ?>" href="<?php echo  $objurl->link('module/pavsliderlayer/layer', 'id='.$slider['id'].'&group_id='.$slider['group_id'].'&lang='.$lang.'&token=' . $objsession->data['token'], 'SSL')?>"><span>Status</span></a>
...
$(document).delegate('a[data-toggle=\'clone\']', 'click', function(e) {
	...
	$('#button-save').on('click', function() { 
		...
		$.ajax({
			url: 'index.php?route=module/pavsliderlayer/cloneGroupSliders&group_id=' + group_id + '&lang=' + lang + '&clonegroup=' + clone_group + '&token=' + getURLVar('token'),
			type: 'GET',
		}).done(function(data) { 
			location.reload(); 
		});
...
$("#btn-update-slider").click( function(){ 
	$('#modal-image').remove();
	$.ajax({
		url: 'index.php?route=module/pavsliderlayer/filemanager&token=' + getURLVar('token') + '&target=' + $("#slider-image").attr('id') + '&thumb=simage',
		dataType: 'html',
