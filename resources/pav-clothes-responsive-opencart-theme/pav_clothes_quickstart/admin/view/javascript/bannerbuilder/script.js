	...
	$.fn.WPO_Layout = function( opts, datajson ) {
		...
		var layoutwidth = config.coldwith*config.col + ((config.col-1)*config.gutter);
	 	...
	 	/**
	  	...
	  	function createBannerWidget( col, img , preview ){
				...
				html += '<input type="hidden" name="banners[]" value="'+img+'" id="input-image'+rand+'" />';

				var banner = $('<div class="inject-banner wpo-ijwidget"></div>');
 			 	$(col).find(".wpo-content").append( banner );
			

				banner.html( "<div class=\"w-inner\">"+html+"</div>" );
			
			 	banner.append('<div class="wpo-wdelete ptstooltip" data-toggle="tooltip" data-placement="top" title="Delete"></div>');
	  			...
	  			$( this.cols ).each( function(){ 
	  				...
	  				$( this.widgets ).each( function(){   
	  				 ...
	  				 //	widgetids.push( this.wkey );
	  				 	if( this.image != null ){
	  				 		createBannerWidget( col, this.image, config.imageurl+this.image )
	  				 	} 
	  				 	console.log( this );
	  				} );
